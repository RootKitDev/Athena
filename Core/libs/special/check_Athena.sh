...
mysql --defaults-extra-file=/home/athena/Core/Files.d/User_SQL.cnf -Bse "exit"  > /dev/null 2>&1
res=$(echo $?)
...
mysql --defaults-extra-file=/home/athena/Core/Files.d/Dumps_User_SQL.cnf -Bse "exit" > /dev/null 2>&1
res=$(echo $?)
if [[ $res == 0 ]]; then
	echo -e "${GREEN}test ... OK${NC}"
else
...
databases=$(echo 'show databases' | mysql --defaults-extra-file=/home/athena/Core/Files.d/Dumps_User_SQL.cnf | grep Athena)

if [[ -n $databases ]]; then
	PROD=$(echo 'show databases' | mysql --defaults-extra-file=/home/athena/Core/Files.d/Dumps_User_SQL.cnf | grep Athena | head -n 1)
	PREPROD=$(echo 'show databases' | mysql --defaults-extra-file=/home/athena/Core/Files.d/Dumps_User_SQL.cnf | grep PREPROD)
	DEV=$(echo 'show databases' | mysql --defaults-extra-file=/home/athena/Core/Files.d/Dumps_User_SQL.cnf | grep DEV)
...
done < /home/athena/Core/Files.d/User_Export.cnf

Hosts=$(mysql --defaults-extra-file=/home/athena/Core/Files.d/User_SQL.cnf -D Athena -e "SELECT host As '' FROM Partners")
for host in ${Hosts[@]}
do
