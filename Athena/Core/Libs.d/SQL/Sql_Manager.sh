...
SQL_save(){
	...
	then
		...
		mysql --defaults-extra-file=$FILE_PATH/User_SQL.cnf -D Athena -e "UPDATE Save SET type=4 WHERE month=$Month and day=$Day"
		State_Save "10"
		...
		do
			...
			SQL_DUMP_CMD="mysqldump --defaults-extra-file=$FILE_PATH/Dumps_User_SQL.cnf '$database' > $EXPORT_PATH/${database}_dump.sql"
