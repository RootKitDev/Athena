...
#########################################################
	...
	REMOTE_HOST_TAB=$(mysql --defaults-extra-file=$FILE_PATH/User_SQL.cnf -D Athena -e "SELECT host As '' FROM Partners")
	i=-1
...
#   2)  SCP_CMD="scp $EXPORT_PATH/$1 $user@$REMOTE_HOST:$REMOTE_EXPORT_PATH"
		SCP_CMD="scp $EXPORT_PATH/$1 $user@$REMOTE_HOST:$REMOTE_EXPORT_PATH"
##################################################################################################

		VarPartners=$(Check_Partners)

		if [[ $VarPartners = 0 ]]; then
...
#   2)      scp $EXPORT_CKSUM_PATH/"Demande_CkSum$SUB_LOG" $user@$REMOTE_HOST:"/path/to/athena"
			scp $EXPORT_CKSUM_PATH/"Demande_CkSum$SUB_LOG" $user@$REMOTE_HOST:"/path/to/athena"
##################################################################################################
			...
			echo "Reprise du script a reception du fichier réponse" >> $LOG_PATH/Save$SUB_LOG.log
			...
			while [ $res -ne 0 ]; do
				sleep 1s
				ls $EXPORT_CKSUM_PATH"/"*"_Remote.cksum" > /dev/null 2>&1
				res=$(echo $?)
			done
	...
	if [[ $i == ${#REMOTE_HOST_TAB[@]} ]] && ([[ "$Old" == "OK" ]] || [[ "$Old" == "SQL_OK" ]]); then
		echo "" >> $LOG_PATH/Save$SUB_LOG.log
